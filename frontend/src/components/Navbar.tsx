...
const Navbar = () => {
  ...
  const navlinkClassName = (path: string) => {
    const defaultClass = 'px-8 text-white hover:bg-primary/80 transition-colors duration-200';
    return `${defaultClass} ` + (currentPath === path ? `font-bold` : '');
  };

  return (
    <nav className="flex flex-row w-lvw shadow-lg justify-center p-4 bg-primary text-white">
